...
const dummyProducts: Product[] = [
  { id: '1', name: 'Fresh Milk', category: 'Dairy', totalOrders: 1245, revenue: 4980 },
  { id: '2', name: 'Whole Wheat Bread', category: 'Bakery', totalOrders: 1120, revenue: 3360 },
  { id: '3', name: 'Chicken Breast', category: 'Meat', totalOrders: 980, revenue: 7840 },
  { id: '4', name: 'Organic Bananas', category: 'Produce', totalOrders: 850, revenue: 2550 },
  { id: '5', name: 'Eggs (Dozen)', category: 'Dairy', totalOrders: 820, revenue: 4100 },
  { id: '6', name: 'Greek Yogurt', category: 'Dairy', totalOrders: 780, revenue: 3120 },
  { id: '7', name: 'Tomatoes', category: 'Produce', totalOrders: 720, revenue: 2160 },
  { id: '8', name: 'Potato Chips', category: 'Snacks', totalOrders: 690, revenue: 2070 },
  { id: '9', name: 'Ground Coffee', category: 'Beverages', totalOrders: 650, revenue: 5200 },
  { id: '10', name: 'Pasta', category: 'Pantry', totalOrders: 630, revenue: 1890 },
];
...
const ProductPopularity: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'products' | 'categories' | 'trends'>('products');
  ...
  const handleMinOrdersChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="p-6 max-w-full">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Product Popularity Analytics</h1>
        <p className="text-gray-600">Track your best-selling products and analyze trends</p>
      </div>
      ...
      <div className="mb-6 border-b">
        <div className="flex gap-6">
          <button
            className={`py-2 px-1 font-medium ${activeTab === 'products' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
            onClick={() => setActiveTab('products')}
          >
            Top Products
          </button>
          <button
            className={`py-2 px-1 font-medium ${activeTab === 'categories' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
            onClick={() => setActiveTab('categories')}
          >
            Category Distribution
          </button>
          <button
            className={`py-2 px-1 font-medium ${activeTab === 'trends' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
          ...
          {activeTab === 'products' && (
            <div className="space-y-6">
              <div className="bg-white p-6 rounded-lg shadow">
                <h3 className="text-lg font-semibold mb-6">Most Popular Products</h3>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <Legend />
                      <Bar dataKey="totalOrders" name="Total Orders" fill="#8884d8" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {categoryData.map((category) => {
                      const totalOrders = categoryData.reduce((sum, cat) => sum + cat.value, 0);
                      const percentage = (category.value / totalOrders * 100).toFixed(1);
            ...
            <div className="space-y-6">
              <div className="bg-white p-6 rounded-lg shadow">
                <h3 className="text-lg font-semibold mb-6">
                  Order Trends {filters.timeRange === 'day' ? 'Today' : 
                               filters.timeRange === 'week' ? 'This Week' : 
                               filters.timeRange === 'month' ? 'This Month' : 'This Year'}
                </h3>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-lg shadow">
                  ...
                  <p className="text-3xl font-bold mt-2">
                    {timeSeriesData.reduce((sum, data) => sum + data.orders, 0).toLocaleString()}
                  </p>
                ...
                <div className="bg-white p-6 rounded-lg shadow">
                  <h3 className="text-base font-medium text-gray-500">Average Daily Orders</h3>
                  <p className="text-3xl font-bold mt-2">
                    {Math.round(timeSeriesData.reduce((sum, data) => sum + data.orders, 0) / timeSeriesData.length).toLocaleString()}
                  </p>
                  <div className="mt-2 text-sm text-green-600">
                ...
                <div className="bg-white p-6 rounded-lg shadow">
                  <h3 className="text-base font-medium text-gray-500">Peak Day</h3>
                  <p className="text-3xl font-bold mt-2">
                    {timeSeriesData.reduce((max, data) => 
                      data.orders > max.orders ? data : max, 
                      { date: '', orders: 0 }
                    ).date}
                  </p>
