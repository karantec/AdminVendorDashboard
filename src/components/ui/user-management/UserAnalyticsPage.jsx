...
const generateActivityData = (days: number): UserActivity[] => {
...
const StatsCard: React.FC<{
  ...
  color?: string;
  isLoading?: boolean;
}> = ({ title, value, icon, trend, color = CHART_COLORS.primary, isLoading = false }) => {
        ...
        {trend && (
          <Box display="flex" alignItems="center" mt={0.5}>
            <Typography 
              variant="body2" 
              sx={{ 
                color: trend.positive ? CHART_COLORS.success : CHART_COLORS.error,
                fontWeight: 500,
                display: 'flex',
                alignItems: 'center'
              }}
...
}> = ({ title, children, height = 350, action }) => {
  const theme = useTheme();
...
const UserAnalyticsPage: React.FC = () => {
  ...
  const totalNewUsers = activityData.reduce((sum, day) => sum + day.newUsers, 0);
  const avgDailyActive = Math.round(activityData.reduce((sum, day) => sum + day.activeUsers, 0) / activityData.length);
  
  // Chart data
  const statusData = [
    { name: 'Active', value: activeUsersCount, color: STATUS_COLORS.active },
    { name: 'Inactive', value: inactiveUsersCount, color: STATUS_COLORS.inactive },
    { name: 'Suspended', value: suspendedUsersCount, color: STATUS_COLORS.suspended }
  ];
  
  const roleData = [
    { name: 'Admin', value: users.filter(user => user.role === 'admin').length, color: ROLE_COLORS.admin },
    { name: 'User', value: users.filter(user => user.role === 'user').length, color: ROLE_COLORS.user },
    { name: 'Moderator', value: users.filter(user => user.role === 'moderator').length, color: ROLE_COLORS.moderator }
  ];
  ...
  const getPreviousPeriodData = () => {
    ...
    const currentActiveUsers = currentPeriodData.reduce((sum, day) => sum + day.activeUsers, 0);
    const previousActiveUsers = previousPeriodData.reduce((sum, day) => sum + day.activeUsers, 0);
    
    const currentNewUsers = currentPeriodData.reduce((sum, day) => sum + day.newUsers, 0);
    const previousNewUsers = previousPeriodData.reduce((sum, day) => sum + day.newUsers, 0);
    
    return {
      activeUsersTrend: {
        value: Math.round(((currentActiveUsers - previousActiveUsers) / previousActiveUsers) * 100),
        positive: currentActiveUsers >= previousActiveUsers
      },
      newUsersTrend: {
        value: Math.round(((currentNewUsers - previousNewUsers) / previousNewUsers) * 100),
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Box sx={{ p: { xs: 2, sm: 3 }, bgcolor: '#F8FAFC', minHeight: '100vh' }}>
        ...
        <Grid container spacing={3} sx={{ mb: 4 }}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <StatsCard
              title={`New Users (${timeRange === '7days' ? '7d' : timeRange === '30days' ? '30d' : '90d'})`}
              ...
              {activityData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={alpha('#000', 0.07)} />
                    <XAxis 
                      dataKey="date" 
        ...
        >
          <CardContent sx={{ p: 0 }}>
            ...
            {filteredUsers.length === 0 ? (
              <EmptyState message={searchTerm ? "No users match your search criteria" : "No users found"} />
            ) : (
              <TableContainer>
