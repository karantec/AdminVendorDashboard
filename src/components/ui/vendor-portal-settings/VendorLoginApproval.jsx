...
import { Search, FilterList, CheckCircle, Cancel, WatchLater } from '@mui/icons-material';
...
const VendorLoginApproval: React.FC = () => {
  // State
    ...
    setTimeout(() => {
      const updatedVendors = vendors.map(vendor => {
        if (vendor.id === selectedVendor.id) {
          return { ...vendor, status: dialogAction === 'approve' ? 'approved' : 'rejected' };
        }
        return vendor;
      });
      ...
      setSnackbar({
        open: true,
        message: `Vendor ${dialogAction === 'approve' ? 'approved' : 'rejected'} successfully!`,
        severity: 'success'
      });
    }, 1000);
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
            ...
            {currentVendors.length > 0 ? (
              currentVendors.map((vendor) => (
                <TableRow key={vendor.id}>
                  ...
                  <TableCell>
                    <Typography fontWeight="medium">{vendor.storeName}</Typography>
                    <Typography variant="body2" color="text.secondary">
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        <DialogTitle>
          {dialogAction === 'approve' ? 'Approve Vendor Access' : 'Reject Vendor Access'}
        </DialogTitle>
        <DialogContent>
          {selectedVendor && (
            <Typography>
        ...
        <DialogActions>
          ...
          >
            {loading ? 'Processing...' : dialogAction === 'approve' ? 'Confirm Approval' : 'Confirm Rejection'}
          </Button>
        </DialogActions>
      </Dialog>
